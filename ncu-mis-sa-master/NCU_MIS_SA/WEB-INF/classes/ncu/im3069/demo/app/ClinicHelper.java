...
public class ClinicHelper {
    ...
    public JSONObject deleteByID(int id) {
        ...
        try {
            ...
            /** SQL指令 */
            String sql = "DELETE FROM `hospital`.`clinic` WHERE `clinic_id` = ? LIMIT 1";
    ...
    public JSONObject getAll() {
        /** 新建一個 Clinic 物件之 c 變數，用於紀錄每一位查詢回之門診資料 */
        Clinic c = null;
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `hospital`.`clinic`";
            ...
            while (rs.next()) {
                ...
                /** 將每一筆門診資料產生一名新Clinic物件 */
                c = new Clinic(clinic_id, clinic_name, dept_id);
                /** 取出該門診之資料並封裝至 JSONsonArray 內 */
                jsa.put(c.getData());
            }
    ...
    public JSONObject getByID(int id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `hosptial`.`clinic` WHERE `clinic`.`clinic_id` = ? LIMIT 1";
     ...
     */
    public boolean checkDuplicate(Clinic c) {
     ...
     */
    public JSONObject create(Clinic c) {
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        /** 紀錄程式開始執行時間 */
        long start_time = System.nanoTime();
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "INSERT INTO `hostipal`.`clinic`(`clinic_id`, `clinic_name`, `dept_id`)"
                    + " VALUES(?, ?, ?)";

            /** 取得所需之參數 */
            int clinic_id = c.getID();
            String clinic_name = c.getName();
            int dept_id = c.getDeptID();

            /** 將參數回填至SQL指令當中 */
            pres = conn.prepareStatement(sql);
            pres.setInt(1, clinic_id);
            pres.setString(2, clinic_name);
            pres.setInt(3, dept_id);
     ...
     */
    public JSONObject update(Clinic c) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `hospital`.`clinic` SET `clinic_name` = ? ,`dept_id` = ? WHERE `clinic_id` = ?";
            /** 取得所需之參數 */
            int clinic_id = c.getID();
            String clinic_name = c.getName();
            int dept_id = c.getDeptID();
