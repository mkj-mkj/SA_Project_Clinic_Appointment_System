...
public class AdministratorHelper {
    ...
    public JSONObject getAll() {
        /** 新建一個 Administrator 物件之 a 變數，用於紀錄每一位查詢回之管理者資料 */
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `hospital`.`Administrator`";
    ...
    public JSONObject getByMail(String mail) {
        ...
        try {
            ...
            /** 透過 while 迴圈移動pointer，取得每一筆回傳資料 */
            /** 正確來說資料庫只會有一筆該admin編號之資料，因此其實可以不用使用 while 迴圈->if */
            if(rs.next()) {
                /** 每執行一次迴圈表示有一筆資料 */
                row += 1;
        ...
        /** 將SQL指令、花費時間、影響行數與所有管理者資料之JSONArray，封裝成JSONObject回傳 */
        JSONObject response = new JSONObject();
        response.put("sql", exexcute_sql);
        response.put("row", row);
        response.put("time", duration);
